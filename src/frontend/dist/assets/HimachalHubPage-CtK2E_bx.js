import { r as reactExports, j as jsxRuntimeExports, L as Link } from "./index-RYfA8Irf.js";
import { S as SectionHeading } from "./SectionHeading-CuY5AD7W.js";
import { T as TrekCard } from "./TrekCard-MEq-mcYL.js";
import { t as treks } from "./treks-NQ8g2zVB.js";
const FEATURED_SLUGS = ["hampta-pass", "sar-pass"];
const difficultyColors = {
  Easy: { bg: "#E8F5EE", text: "#1A7A4C" },
  "Easy to Moderate": { bg: "#E8F5EE", text: "#1A7A4C" },
  Moderate: { bg: "#FFF3CD", text: "#856404" },
  Difficult: { bg: "#FDECEA", text: "#C0392B" }
};
function HimachalHubPage() {
  const hpTreks = reactExports.useMemo(
    () => treks.filter((t) => t.state === "Himachal Pradesh"),
    []
  );
  const featuredTreks = reactExports.useMemo(
    () => FEATURED_SLUGS.map((s) => hpTreks.find((t) => t.slug === s)).filter(
      Boolean
    ),
    [hpTreks]
  );
  const gridTreks = reactExports.useMemo(
    () => hpTreks.filter((t) => !FEATURED_SLUGS.includes(t.slug)),
    [hpTreks]
  );
  const infoCards = [
    {
      icon: "🌊",
      title: "Parvati Valley Treks",
      desc: "Sar Pass and Kheerganga wind through the mystical Parvati Valley — pine-scented forest trails, hot springs and Himalayan villages."
    },
    {
      icon: "🗻",
      title: "Manali-Based Treks",
      desc: "Hampta Pass and Beas Kund are iconic Manali treks through Kullu Valley, with dramatic crossovers into the arid Lahaul landscape."
    },
    {
      icon: "🌸",
      title: "Best Seasons",
      desc: "June–September is ideal — monsoon brings lush meadows and roaring waterfalls. April–May has spectacular snow on Sar Pass and Hampta."
    },
    {
      icon: "🏔️",
      title: "Altitude & Access",
      desc: "Treks range 2,850m–4,270m. Base cities are Manali, Kasol, and Dharamshala — all well connected by road from Chandigarh."
    }
  ];
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { paddingTop: "var(--nav-height, 72px)" }, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        style: {
          background: "linear-gradient(135deg, #0a3320 0%, #1A7A4C 55%, #2ECC71 100%)",
          padding: "80px 24px 64px",
          position: "relative",
          overflow: "hidden"
        },
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              "aria-hidden": "true",
              style: {
                position: "absolute",
                top: "-80px",
                right: "-60px",
                width: "380px",
                height: "380px",
                borderRadius: "50%",
                background: "rgba(255,255,255,0.04)"
              }
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              "aria-hidden": "true",
              style: {
                position: "absolute",
                bottom: "-50px",
                left: "15%",
                width: "260px",
                height: "260px",
                borderRadius: "50%",
                background: "rgba(46,204,113,0.07)"
              }
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              style: { maxWidth: "1200px", margin: "0 auto", position: "relative" },
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("nav", { "aria-label": "Breadcrumb", style: { marginBottom: "20px" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "ol",
                  {
                    style: {
                      display: "flex",
                      alignItems: "center",
                      gap: "6px",
                      listStyle: "none",
                      padding: 0,
                      margin: 0
                    },
                    children: ["Home", "Treks", "Himachal Pradesh"].map((crumb, i, arr) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "li",
                      {
                        style: { display: "flex", alignItems: "center", gap: "6px" },
                        children: [
                          i > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "span",
                            {
                              style: {
                                color: "rgba(255,255,255,0.5)",
                                fontSize: "13px"
                              },
                              children: "›"
                            }
                          ),
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "span",
                            {
                              style: {
                                fontFamily: "var(--font-body)",
                                fontSize: "13px",
                                color: i === arr.length - 1 ? "#fff" : "rgba(255,255,255,0.7)",
                                fontWeight: i === arr.length - 1 ? 600 : 400
                              },
                              children: crumb
                            }
                          )
                        ]
                      },
                      crumb
                    ))
                  }
                ) }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "div",
                  {
                    style: {
                      display: "flex",
                      alignItems: "flex-start",
                      gap: "48px",
                      flexWrap: "wrap"
                    },
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { flex: "1 1 400px" }, children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsxs(
                          "div",
                          {
                            style: {
                              display: "inline-flex",
                              alignItems: "center",
                              gap: "8px",
                              background: "rgba(255,255,255,0.12)",
                              borderRadius: "50px",
                              padding: "5px 14px",
                              marginBottom: "20px"
                            },
                            children: [
                              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: "16px" }, children: "🌲" }),
                              /* @__PURE__ */ jsxRuntimeExports.jsx(
                                "span",
                                {
                                  style: {
                                    fontFamily: "var(--font-body)",
                                    fontSize: "12px",
                                    fontWeight: 700,
                                    color: "#fff",
                                    letterSpacing: "0.1em",
                                    textTransform: "uppercase"
                                  },
                                  children: "Himachal Pradesh Trekking"
                                }
                              )
                            ]
                          }
                        ),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "h1",
                          {
                            style: {
                              fontFamily: "var(--font-display)",
                              fontSize: "clamp(36px, 5vw, 60px)",
                              fontWeight: 700,
                              color: "#fff",
                              lineHeight: 1.1,
                              margin: "0 0 20px"
                            },
                            children: "Himachal Pradesh Treks"
                          }
                        ),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "p",
                          {
                            style: {
                              fontFamily: "var(--font-body)",
                              fontSize: "clamp(15px, 1.8vw, 18px)",
                              color: "rgba(255,255,255,0.88)",
                              lineHeight: 1.6,
                              maxWidth: "540px",
                              margin: "0 0 32px"
                            },
                            children: "5 iconic treks through Parvati Valley's mystical forests, Manali's dramatic crossover passes, and Dharamshala's Dhauladhar ridgelines."
                          }
                        ),
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: "12px", flexWrap: "wrap" }, children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "a",
                            {
                              href: "#all-treks",
                              "data-ocid": "hp_hub.explore_button",
                              style: {
                                display: "inline-flex",
                                alignItems: "center",
                                gap: "8px",
                                background: "#fff",
                                color: "#1A7A4C",
                                fontFamily: "var(--font-body)",
                                fontWeight: 700,
                                fontSize: "15px",
                                padding: "12px 28px",
                                borderRadius: "10px",
                                textDecoration: "none",
                                transition: "all 0.2s"
                              },
                              children: "Explore All 5 Treks"
                            }
                          ),
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "a",
                            {
                              href: "/contact",
                              "data-ocid": "hp_hub.inquiry_button",
                              style: {
                                display: "inline-flex",
                                alignItems: "center",
                                gap: "8px",
                                background: "rgba(255,255,255,0.12)",
                                color: "#fff",
                                border: "1.5px solid rgba(255,255,255,0.4)",
                                fontFamily: "var(--font-body)",
                                fontWeight: 600,
                                fontSize: "15px",
                                padding: "12px 28px",
                                borderRadius: "10px",
                                textDecoration: "none",
                                transition: "all 0.2s"
                              },
                              children: "Plan My Trek"
                            }
                          )
                        ] })
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "div",
                        {
                          style: {
                            display: "grid",
                            gridTemplateColumns: "1fr 1fr",
                            gap: "12px",
                            flexShrink: 0
                          },
                          children: [
                            { value: "5", label: "HP Treks" },
                            { value: "2,850m–4,270m", label: "Altitude Range" },
                            { value: "2–5 Days", label: "Duration Range" },
                            { value: "4.8★", label: "Avg. Rating" }
                          ].map((s) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                            "div",
                            {
                              style: {
                                background: "rgba(255,255,255,0.1)",
                                borderRadius: "12px",
                                padding: "16px 20px",
                                backdropFilter: "blur(8px)",
                                border: "1px solid rgba(255,255,255,0.15)",
                                minWidth: "130px"
                              },
                              children: [
                                /* @__PURE__ */ jsxRuntimeExports.jsx(
                                  "div",
                                  {
                                    style: {
                                      fontFamily: "var(--font-display)",
                                      fontSize: "22px",
                                      fontWeight: 700,
                                      color: "#fff",
                                      lineHeight: 1
                                    },
                                    children: s.value
                                  }
                                ),
                                /* @__PURE__ */ jsxRuntimeExports.jsx(
                                  "div",
                                  {
                                    style: {
                                      fontFamily: "var(--font-body)",
                                      fontSize: "12px",
                                      color: "rgba(255,255,255,0.75)",
                                      marginTop: "4px"
                                    },
                                    children: s.label
                                  }
                                )
                              ]
                            },
                            s.label
                          ))
                        }
                      )
                    ]
                  }
                )
              ]
            }
          )
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { background: "#fff", padding: "64px 24px" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { maxWidth: "1200px", margin: "0 auto" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        SectionHeading,
        {
          label: "Signature Routes",
          title: "Featured Himachal Treks",
          subtitle: "Hampta Pass and Sar Pass are two of India's most celebrated mountain crossovers — dramatic, breathtaking, and unforgettable."
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "div",
        {
          style: {
            display: "grid",
            gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))",
            gap: "28px",
            marginBottom: "24px"
          },
          children: featuredTreks.map((trek, i) => {
            var _a;
            if (!trek) return null;
            const diffStyle = difficultyColors[trek.difficulty] ?? difficultyColors.Moderate;
            return /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "article",
              {
                "data-ocid": `hp_hub.featured_card.${i + 1}`,
                style: {
                  background: "#fff",
                  borderRadius: "16px",
                  border: "1px solid #E8F5EE",
                  borderTop: "4px solid #1A7A4C",
                  boxShadow: "0 4px 24px rgba(26,122,76,0.1)",
                  overflow: "hidden",
                  transition: "all 0.3s"
                },
                onMouseEnter: (e) => {
                  const el = e.currentTarget;
                  el.style.transform = "translateY(-4px)";
                  el.style.boxShadow = "0 16px 40px rgba(26,122,76,0.18)";
                },
                onMouseLeave: (e) => {
                  const el = e.currentTarget;
                  el.style.transform = "translateY(0)";
                  el.style.boxShadow = "0 4px 24px rgba(26,122,76,0.1)";
                },
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "div",
                    {
                      style: {
                        position: "relative",
                        height: "240px",
                        overflow: "hidden",
                        background: "linear-gradient(135deg, #E8F5EE, #1A7A4C33)"
                      },
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "img",
                          {
                            src: "/assets/generated/himalaya-hero.dim_1600x900.jpg",
                            alt: `${trek.name} in Himachal Pradesh`,
                            style: {
                              width: "100%",
                              height: "100%",
                              objectFit: "cover",
                              transition: "transform 0.4s"
                            },
                            loading: "lazy"
                          }
                        ),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "div",
                          {
                            style: {
                              position: "absolute",
                              inset: 0,
                              background: "linear-gradient(to top, rgba(20,92,56,0.65) 0%, transparent 55%)"
                            }
                          }
                        ),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "div",
                          {
                            style: {
                              position: "absolute",
                              bottom: "16px",
                              left: "16px",
                              right: "16px"
                            },
                            children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                              "div",
                              {
                                style: {
                                  display: "flex",
                                  justifyContent: "space-between",
                                  alignItems: "flex-end"
                                },
                                children: [
                                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                                    "h3",
                                    {
                                      style: {
                                        fontFamily: "var(--font-display)",
                                        fontSize: "22px",
                                        fontWeight: 700,
                                        color: "#fff",
                                        margin: 0,
                                        lineHeight: 1.2
                                      },
                                      children: trek.name
                                    }
                                  ),
                                  /* @__PURE__ */ jsxRuntimeExports.jsxs(
                                    "span",
                                    {
                                      style: {
                                        background: "rgba(20,92,56,0.9)",
                                        color: "#fff",
                                        borderRadius: "8px",
                                        padding: "4px 10px",
                                        fontFamily: "var(--font-body)",
                                        fontWeight: 700,
                                        fontSize: "13px",
                                        whiteSpace: "nowrap"
                                      },
                                      children: [
                                        "₹",
                                        trek.priceFrom.toLocaleString("en-IN"),
                                        "+"
                                      ]
                                    }
                                  )
                                ]
                              }
                            )
                          }
                        ),
                        trek.isTrending && /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "div",
                          {
                            style: {
                              position: "absolute",
                              top: "12px",
                              right: "12px",
                              background: "#F4A623",
                              color: "#fff",
                              fontSize: "11px",
                              fontWeight: 700,
                              padding: "4px 10px",
                              borderRadius: "50px"
                            },
                            children: "🔥 Trending"
                          }
                        )
                      ]
                    }
                  ),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { padding: "22px 24px 24px" }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "p",
                      {
                        style: {
                          fontFamily: "var(--font-body)",
                          fontSize: "14px",
                          color: "#4A4A4A",
                          lineHeight: 1.6,
                          marginBottom: "16px"
                        },
                        children: trek.shortDescription
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "div",
                      {
                        style: {
                          display: "flex",
                          gap: "8px",
                          flexWrap: "wrap",
                          marginBottom: "20px"
                        },
                        children: [
                          [
                            { icon: "⏱", val: trek.durationLabel },
                            {
                              icon: "📍",
                              val: `${trek.maxAltitudeM.toLocaleString()}m`
                            },
                            {
                              icon: "🗓",
                              val: ((_a = trek.bestSeason.split(";")) == null ? void 0 : _a[0]) ?? trek.bestSeason
                            }
                          ].map((s) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                            "span",
                            {
                              style: {
                                display: "inline-flex",
                                alignItems: "center",
                                gap: "4px",
                                background: "#F8FAF9",
                                border: "1px solid #E8F5EE",
                                borderRadius: "50px",
                                padding: "4px 12px",
                                fontFamily: "var(--font-body)",
                                fontSize: "12px",
                                color: "#1A1A1A",
                                fontWeight: 600
                              },
                              children: [
                                s.icon,
                                " ",
                                s.val
                              ]
                            },
                            s.icon
                          )),
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "span",
                            {
                              style: {
                                display: "inline-flex",
                                alignItems: "center",
                                gap: "4px",
                                background: diffStyle.bg,
                                borderRadius: "50px",
                                padding: "4px 12px",
                                fontFamily: "var(--font-body)",
                                fontSize: "12px",
                                color: diffStyle.text,
                                fontWeight: 700
                              },
                              children: trek.difficulty
                            }
                          )
                        ]
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "ul",
                      {
                        style: {
                          listStyle: "none",
                          padding: 0,
                          margin: "0 0 20px",
                          display: "flex",
                          flexDirection: "column",
                          gap: "6px"
                        },
                        children: trek.highlights.slice(0, 3).map((h) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                          "li",
                          {
                            style: {
                              display: "flex",
                              alignItems: "flex-start",
                              gap: "8px",
                              fontFamily: "var(--font-body)",
                              fontSize: "13px",
                              color: "#4A4A4A",
                              lineHeight: 1.4
                            },
                            children: [
                              /* @__PURE__ */ jsxRuntimeExports.jsx(
                                "span",
                                {
                                  style: {
                                    color: "#1A7A4C",
                                    fontWeight: 700,
                                    flexShrink: 0,
                                    marginTop: "1px"
                                  },
                                  children: "✓"
                                }
                              ),
                              h
                            ]
                          },
                          h
                        ))
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: "10px" }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        Link,
                        {
                          to: `/treks/himachal-pradesh/${trek.slug}`,
                          "data-ocid": `hp_hub.featured_link.${i + 1}`,
                          style: {
                            flex: 1,
                            display: "flex",
                            alignItems: "center",
                            justifyContent: "center",
                            padding: "10px",
                            border: "1.5px solid #1A7A4C",
                            borderRadius: "8px",
                            color: "#1A7A4C",
                            fontFamily: "var(--font-body)",
                            fontWeight: 600,
                            fontSize: "13px",
                            textDecoration: "none"
                          },
                          children: "Full Details"
                        }
                      ),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        Link,
                        {
                          to: `/treks/himachal-pradesh/${trek.slug}`,
                          "data-ocid": `hp_hub.featured_book.${i + 1}`,
                          style: {
                            flex: 1,
                            display: "flex",
                            alignItems: "center",
                            justifyContent: "center",
                            padding: "10px",
                            background: "#F4A623",
                            borderRadius: "8px",
                            color: "#fff",
                            fontFamily: "var(--font-body)",
                            fontWeight: 700,
                            fontSize: "13px",
                            textDecoration: "none"
                          },
                          children: "Book Now"
                        }
                      )
                    ] })
                  ] })
                ]
              },
              trek.slug
            );
          })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          style: {
            background: "linear-gradient(to right, #E8F5EE, #F8FAF9)",
            border: "1px solid #D1E7D9",
            borderLeft: "4px solid #1A7A4C",
            borderRadius: "12px",
            padding: "24px 28px",
            display: "flex",
            alignItems: "center",
            gap: "20px",
            flexWrap: "wrap"
          },
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: "36px" }, children: "🪷" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { flex: 1 }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "h4",
                {
                  style: {
                    fontFamily: "var(--font-body)",
                    fontSize: "16px",
                    fontWeight: 700,
                    color: "#1A1A1A",
                    margin: "0 0 4px"
                  },
                  children: "Parvati Valley — India's Most Mystical Trekking Corridor"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "p",
                {
                  style: {
                    fontFamily: "var(--font-body)",
                    fontSize: "14px",
                    color: "#4A4A4A",
                    margin: 0,
                    lineHeight: 1.5
                  },
                  children: "From Kasol's cafes to Kheerganga's hot springs, Sar Pass, and beyond — the Parvati Valley offers an unmatched blend of natural beauty, cultural depth, and trekking adventure."
                }
              )
            ] })
          ]
        }
      )
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        id: "all-treks",
        style: { background: "#F8FAF9", padding: "64px 24px 80px" },
        children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { maxWidth: "1200px", margin: "0 auto" }, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            SectionHeading,
            {
              label: "Complete Collection",
              title: "All 5 Himachal Pradesh Treks",
              subtitle: "Every route — from easy Triund to dramatic Hampta Pass — curated for maximum adventure."
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              "data-ocid": "hp_hub.treks_list",
              style: {
                display: "grid",
                gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))",
                gap: "24px"
              },
              children: gridTreks.map((trek, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(TrekCard, { trek, index: i }, trek.slug))
            }
          )
        ] })
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { background: "#fff", padding: "64px 24px 80px" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { maxWidth: "1200px", margin: "0 auto" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        SectionHeading,
        {
          label: "Why Himachal Pradesh",
          title: "Trekking in Himachal Pradesh",
          subtitle: "From Manali to Dharamshala to Kasol — Himachal's valleys offer diverse, dramatic, and deeply personal Himalayan experiences.",
          centered: true
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "div",
        {
          style: {
            display: "grid",
            gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))",
            gap: "20px",
            marginBottom: "48px"
          },
          children: infoCards.map((card) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              style: {
                background: "#F8FAF9",
                border: "1px solid #E8F5EE",
                borderTop: "3px solid #1A7A4C",
                borderRadius: "12px",
                padding: "24px 20px"
              },
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: "28px", marginBottom: "12px" }, children: card.icon }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "h3",
                  {
                    style: {
                      fontFamily: "var(--font-body)",
                      fontSize: "16px",
                      fontWeight: 700,
                      color: "#1A1A1A",
                      marginBottom: "8px"
                    },
                    children: card.title
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                    style: {
                      fontFamily: "var(--font-body)",
                      fontSize: "14px",
                      color: "#4A4A4A",
                      lineHeight: 1.55,
                      margin: 0
                    },
                    children: card.desc
                  }
                )
              ]
            },
            card.title
          ))
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          style: {
            background: "linear-gradient(135deg, #145C38, #1A7A4C)",
            borderRadius: "20px",
            padding: "48px",
            textAlign: "center"
          },
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "h3",
              {
                style: {
                  fontFamily: "var(--font-display)",
                  fontSize: "clamp(24px, 3vw, 32px)",
                  fontWeight: 700,
                  color: "#fff",
                  marginBottom: "12px"
                },
                children: "Start Your Himachal Adventure Today"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "p",
              {
                style: {
                  fontFamily: "var(--font-body)",
                  fontSize: "16px",
                  color: "rgba(255,255,255,0.85)",
                  marginBottom: "28px",
                  maxWidth: "460px",
                  margin: "0 auto 28px"
                },
                children: "Expert local guides, fully managed camping, and unforgettable mountain crossovers."
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                style: {
                  display: "flex",
                  gap: "12px",
                  justifyContent: "center",
                  flexWrap: "wrap"
                },
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "a",
                    {
                      href: "/treks",
                      "data-ocid": "hp_hub.cta_treks_button",
                      style: {
                        display: "inline-flex",
                        alignItems: "center",
                        padding: "13px 32px",
                        background: "#fff",
                        color: "#1A7A4C",
                        borderRadius: "10px",
                        fontFamily: "var(--font-body)",
                        fontWeight: 700,
                        fontSize: "15px",
                        textDecoration: "none"
                      },
                      children: "All Treks"
                    }
                  ),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "a",
                    {
                      href: "/contact",
                      "data-ocid": "hp_hub.cta_contact_button",
                      style: {
                        display: "inline-flex",
                        alignItems: "center",
                        padding: "13px 32px",
                        background: "rgba(255,255,255,0.12)",
                        color: "#fff",
                        border: "1.5px solid rgba(255,255,255,0.4)",
                        borderRadius: "10px",
                        fontFamily: "var(--font-body)",
                        fontWeight: 600,
                        fontSize: "15px",
                        textDecoration: "none"
                      },
                      children: "Get Expert Advice"
                    }
                  )
                ]
              }
            )
          ]
        }
      )
    ] }) })
  ] });
}
export {
  HimachalHubPage
};
